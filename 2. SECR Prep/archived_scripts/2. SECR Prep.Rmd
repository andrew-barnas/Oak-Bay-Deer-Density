The purpose of this script is to take the outputs from 1. Data Prep and format the data for input into the SECR models. This is similar to the data prep step before, but specialized for secr. The main outputs of this for each year are:
1. A capture list of knnown individuals
...
input_directory<-reproducible::checkPath(file.path(getwd(), "1. Data Prep/outputs"), create = TRUE)
...
deer_2020<-read.csv(file.path(input_directory,"marked_detections_sept_2020.csv"))
...
unmarked_2020<-read.csv(file.path(input_directory,"unmarked_matrix_sept_2020.csv"))
...
usage_2020<-read.csv(file.path(input_directory,"operation_matrix_sept_2020.csv"))
...
Lets deal with this shit year by year, so you don't have to run through everything each time.
2018 first
#######################################################################################
...
cams_2018<-cams_2018%>%
  dplyr::select(-c(x, y))%>%
  dplyr::select(trap.id, x = cams_2018.y, y=cams_2018.x) #I dont know WHY, but the x and y come out backwards...
