I am cleaning and preparing the data for each year seperately, as each year has its own unique issues to be addressed. While somewhat redundant, this is ultimately easier.

The purpose of this script is to prepare the 2018 deer detection data for density estimation using secr. The main ouputs of this script are:
1. a file of the marked detections in 2018
2. a file of the unmarked detections in 2018
3. a camera operability matrix for september 2018
...
deer_2018<-read.csv(file.path(getwd(), "1.1 Assigning WHIDs/outputs/main_dataframe_with_whid.csv"))
...
#What if i just want 2018 stuff
deer_2018<-deer_2018%>%
...
str(deer_2018$datetime) #remember this is a datetime, not just a date. 
```
...
september_interval <-lubridate::interval(start = "2018-09-01", end = "2018-09-30")
...
camop_2018_figure<-ggplot(cam_deploy%>%
  mutate(new_start = case_when(start_date < ymd("2018-09-01") ~ ymd("2018-09-01"),
                               start_date > ymd("2018-09-01") ~ start_date),
         new_end = case_when(end_date > ymd("2018-09-30") ~ ymd("2018-09-30"),
                               end_date < ymd("2018-09-30") ~ end_date)), 
  aes(y = SiteID))+
  geom_segment(aes(x = new_start, xend = new_end,
                   y = SiteID, yend=SiteID))+
  geom_vline(xintercept = ymd("2018-09-01"), linetype = "dashed", color = "red", linewidth = 1)+
  geom_vline(xintercept = ymd("2018-09-30"), linetype = "dashed", color = "red", linewidth = 1)+
  xlab("Date")+
  ylab("Site ID")+
...
ggsave(filename = "camera_operability_matrix_2018.jpeg", #name of file
       plot = camop_2018_figure,                 #plot you want to save
...
A<-cam_deploy%>%
  mutate(new_start = case_when(start_date <= ymd("2018-09-01") ~ ymd("2018-09-01"),
                               start_date > ymd("2018-09-01")  ~ start_date),
         new_end = case_when(end_date <= ymd("2018-09-30") ~ end_date,
                               end_date > ymd("2018-09-30")  ~ ymd("2018-09-30")),
...
cam_summary_2018<-cam_deploy%>%
  ...
  summarise(n_cams = n_distinct(SiteID),
            ...
            min_n = min(n_days),
            max_n = max(n_days))
write.csv(cam_summary_2018, file.path(output_directory, "camera_operability_summary_2018.csv"),
          row.names = FALSE)
...
cam_deploy<-cam_deploy %>%
  ...
  select(c(1,`2018-09-01`:`2018-09-30`))%>%
  replace(is.na(.), 0)
...
write.csv(cam_deploy, file.path(output_directory, "operation_matrix_sept_2018.csv"), row.names = FALSE)
...
n_distinct(deer_2018$SiteID) #33 sites (looks like one site had zero detections, strange, asked Sandra)
n_distinct(cam_deploy$SiteID) #34 sites

#Remove rows where date is outside of September 2018
#deer_2018_original<-deer_2018 %>%
 # filter(datetime >= "2018-09-01" & datetime < "2018-10-01")
  #filter(date(datetime) >= "2018-09-01" & date(datetime) < "2018-10-01")
...
#How many deer?
#unique(deer_2018_original$WHID_all)
#unique(deer_2018_new$WHID_all)


colnames(deer_2018)

#Ok at this point, I want to output the current dataframe to a seperate dataframe, because here I remove fawns
write.csv(deer_2018, file.path(fawn_data_directory, "deer_2018_with_fawns.csv"))
