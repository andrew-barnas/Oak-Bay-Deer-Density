...
#3 Detection file - this one has all the wildlife health IDS fixed - received from Mac G April 30th 2024
deer_2023<-read.csv(file.path(getwd(), "1.1 Assigning WHIDs/outputs/main_dataframe_with_whid.csv"))
...
str(deer_2023$datetime) #remember this is a datetime, not just a date. 
...
september_interval <-lubridate::interval(start = "2023-09-01", end = "2023-09-30")
...
camop_2023_figure<-ggplot(cam_deploy%>%
  mutate(new_start = case_when(start_date < ymd("2023-09-01") ~ ymd("2023-09-01"),
                               start_date > ymd("2023-09-01") ~ start_date),
         new_end = case_when(end_date > ymd("2023-09-30") ~ ymd("2023-09-30"),
                               end_date < ymd("2023-09-30") ~ end_date)), 
  aes(y = SiteID))+
  geom_segment(aes(x = new_start, xend = new_end,
                   y = SiteID, yend=SiteID))+
  geom_vline(xintercept = ymd("2023-09-01"), linetype = "dashed", color = "red", linewidth = 1)+
  geom_vline(xintercept = ymd("2023-09-30"), linetype = "dashed", color = "red", linewidth = 1)+
  xlab("Date")+
  ylab("Site ID")+
...
ggsave(filename = "camera_operability_matrix_2023.jpeg", #name of file
       plot = camop_2023_figure,                 #plot you want to save
...
A<-cam_deploy%>%
  mutate(new_start = case_when(start_date <= ymd("2023-09-01") ~ ymd("2023-09-01"),
                               start_date > ymd("2023-09-01")  ~ start_date),
         new_end = case_when(end_date <= ymd("2023-09-30") ~ end_date,
                               end_date > ymd("2023-09-30")  ~ ymd("2023-09-30")),
...
cam_summary_2023<-cam_deploy%>%
  ...
  summarise(n_cams = n_distinct(SiteID),
            ...
            min_n = min(n_days),
            max_n = max(n_days))
write.csv(cam_summary_2023, file.path(output_directory, "camera_operability_summary_2023.csv"),
          row.names = FALSE)
...
cam_deploy<-cam_deploy %>%
  ...
  select(c(1,`2023-09-01`:`2023-09-30`))%>%
  replace(is.na(.), 0)
...
write.csv(cam_deploy, file.path(output_directory, "operation_matrix_sept_2023.csv"), row.names = FALSE)
...
n_distinct(deer_2023$SiteID) #32 sites (looks like two site had zero detections)
n_distinct(cam_deploy$SiteID) #34 sites

#CHECKING FOR MY MISSING DEER
deer_2023%>%
  filter(WHID_all == "['19-0697']")%>%
  mutate(inside = date(datetime) >= "2023-09-01")

#Remove rows where date is outside of September 2023
deer_2023<-deer_2023 %>%
  filter(date(datetime) >= "2023-09-01" & date(datetime) < "2023-10-01")

colnames(deer_2023)


#Ok at this point, I want to output the current dataframe to a seperate dataframe, because here I remove fawns
write.csv(deer_2023, file.path(fawn_data_directory, "deer_2023_with_fawns.csv"))
...
#Ok and at this point we are still good with 30 individuals!
unique(deer_2023$WHID_all)
n_distinct(deer_2023$WHID_all)
...
unique(deer_2023_unmarked$WHID_all)
deer_2023_unmarked<-deer_2023_unmarked%>%
  mutate(WHID_all = "")
...
deer_2023_marked<-deer_2023_marked%>%
  filter(str_length(WHID_all) <= 12)%>%
  #but also remove that one blank
  filter(WHID_all != "")
...
deer_2023_marked<-bind_rows(deer_2023_marked, new_df)
unique(deer_2023_marked$WHID_all)
n_distinct(deer_2023_marked$WHID_all)
```
...
deer_2023_marked$Event.ID <- 9999
mins <- 30
...
#In this case, zero.
seq <- as.numeric(paste0(nrow(deer_2023_marked),0))
seq <- round(seq,-(nchar(seq)))

for (i in 2:nrow(deer_2023_marked)) {
  #Starting at the first row, assign an event ID that is (E+seq)
  deer_2023_marked$Event.ID[i-1]  <- paste0("E",format(seq, scientific = F)) 
  ...
  if(is.na(deer_2023_marked$duration[i]) | abs(deer_2023_marked$duration[i]) > mins){
    seq <- seq + 1 
  }
}
...
if(deer_2023_marked$duration[nrow(deer_2023_marked)] < mins| 
   is.na(deer_2023_marked$duration[nrow(deer_2023_marked)])){
  deer_2023_marked$Event.ID[nrow(deer_2023_marked)] <- deer_2023_marked$Event.ID[nrow(deer_2023_marked)-1] 
} else{
  deer_2023_marked$Event.ID[nrow(deer_2023_marked)] <- paste0("E",format(seq+1, scientific = F)) 
}
...
deer_2023_unmarked$Event.ID <- 9999
mins <- 30   # THIS IS THE DETECTION EVENT BREAK-POINT YOU CAN CHANGE
seq <- as.numeric(paste0(nrow(deer_2023_unmarked),0))
seq <- round(seq,-(nchar(seq)))

for (i in 2:nrow(deer_2023_unmarked)) {
  deer_2023_unmarked$Event.ID[i-1]  <- paste0("E",format(seq, scientific = F))
  if(is.na(deer_2023_unmarked$duration[i]) | abs(deer_2023_unmarked$duration[i]) > mins){
    seq <- seq + 1 
  }
}
...
# Update the information for the last row
# group ID  for the last row
if(deer_2023_unmarked$duration[nrow(deer_2023_unmarked)] < mins| 
   is.na(deer_2023_unmarked$duration[nrow(deer_2023_unmarked)])){
  deer_2023_unmarked$Event.ID[nrow(deer_2023_unmarked)] <- deer_2023_unmarked$Event.ID[nrow(deer_2023_unmarked)-1] 
} else{
  deer_2023_unmarked$Event.ID[nrow(deer_2023_unmarked)] <- paste0("E",format(seq+1, scientific = F)) 
}
...
write.csv(deer_2023_marked, file.path(output_directory, "marked_detections_sept_2023.csv"), row.names = FALSE)
...
A<-merge(A, deer_2023_unmarked, by = c("trap.id", "occassion"), all.x = TRUE)
...
write.csv(deer_2023_unmarked, file.path(output_directory, "unmarked_matrix_sept_2023.csv"), row.names = FALSE)
```
