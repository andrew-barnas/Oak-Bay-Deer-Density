...
3. a camera operability matrix for October 8th to November 6th 2019
...
deer_2019<-read.csv(file.path(getwd(), "1.1 Assigning WHIDs/outputs/main_dataframe_with_whid.csv"))
...
#I ALSO ONLY WANT STUFF FROM 2019 HERE
deer_2019<-deer_2019%>%
...
A<-deer_2019%>%
  ...
  select(Date, Time, datetime, datetime2)%>%
  ...
  mutate(datetime3 = case_when(
    year == 2019 ~ datetime2,
    year != 2019 ~ dmy_hms(datetime)))
...
#  select(Date, Time, datetime)%>%
  ...
  mutate(datetime = case_when(
    year == 2019 ~ datetime,
    year != 2019 ~ dmy_hms(paste(Date, Time, sep = " "))))
    

#Lets look at these datetimes just to be sure
ggplot(deer_2019, aes(x = datetime))+
  geom_histogram()
...
min(deer_2019$datetime)
max(deer_2019$datetime)
#hist(deer_2019$Date)
...
october_november_interval <-lubridate::interval(start = "2019-10-08", end = "2019-11-06")
#And then figure out if the start-end date interval overlaps the 2019 interval
#This is the list of cameras that we could possibly have a detection in the 2019 interval
cam_deploy<-cam_deploy%>%
...
#Need to do something tricky to the dataframe. Basically need it so that if the camera started before Oct 8th, we change the start date to Oct 8th, and if it ended after Nov 6th, we change it to Nov 6th 


#I THINK I SAVE THIS AS A SUPPLEMENTAL MATERIAL??
camop_2019_figure<-ggplot(cam_deploy%>%
  mutate(new_start = case_when(start_date <= ymd("2019-10-08") ~ ymd("2019-10-08"),
                               start_date > ymd("2019-10-08") ~ start_date),
         new_end = case_when(end_date >= ymd("2019-11-06") ~ ymd("2019-11-06"),
                               end_date < ymd("2019-11-06") ~ end_date)), 
  aes(y = SiteID))+
  geom_segment(aes(x = new_start, xend = new_end,
                   y = SiteID, yend=SiteID))+
  geom_vline(xintercept = ymd("2019-10-08"), linetype = "dashed", color = "red", linewidth = 1)+
  geom_vline(xintercept = ymd("2019-11-06"), linetype = "dashed", color = "red", linewidth = 1)+
  xlab("Date")+
  ylab("Site ID")+
  ggtitle("Fall 2019")
...
A<-cam_deploy%>%
  mutate(new_start = case_when(start_date <= ymd("2019-10-08") ~ ymd("2019-10-08"),
                               start_date > ymd("2019-10-08")  ~ start_date),
         new_end = case_when(end_date <= ymd("2019-11-06") ~ end_date,
                               end_date > ymd("2019-11-06")  ~ ymd("2019-11-06")),
...
#Ahhhh Site 23 and Site 26 were in the Septermber data but here we are using the October data. I think we can safely remove them from the detection file at this point.
ggplot(deer_2019%>%
         filter(SiteID == "Site23"), aes(x = datetime))+
  geom_histogram()

ggplot(deer_2019%>%
         filter(SiteID == "Site26"), aes(x = datetime))+
  geom_histogram()

deer_2019<-deer_2019%>%
  filter(SiteID != "Site23")%>%
  filter(SiteID != "Site26")
...
cam_deploy<-cam_deploy %>%
  ...
  select(c(1,`2019-10-08`:`2019-11-06`))%>%
  replace(is.na(.), 0)
...
write.csv(cam_deploy, file.path(output_directory, "operation_matrix_fall_2019.csv"), row.names = FALSE)
...
n_distinct(deer_2019$SiteID) #31 sites (looks like one site had zero detections, strange, asked Sandra)
n_distinct(cam_deploy$SiteID) #32 sites

#Remove rows where date is outside of the 2019 Fall window
deer_2019<-deer_2019 %>%
  #filter(datetime >= "2018-09-01" & datetime < "2018-10-01")
  filter(date(datetime) >= "2019-10-08" & date(datetime) < "2019-11-07")

colnames(deer_2019)

#Ok at this point, I want to output the current dataframe to a seperate dataframe, because here I remove fawns
write.csv(deer_2019, file.path(fawn_data_directory, "deer_2019_with_fawns.csv"))
...
unique(deer_2019_unmarked$WHID_all)
deer_2019_unmarked<-deer_2019_unmarked%>%
  mutate(WHID_all = "")
...
deer_2019_marked<-deer_2019_marked %>%
...
deer_2019_marked$Event.ID <- 9999
mins <- 30
...
#In this case, zero.
seq <- as.numeric(paste0(nrow(deer_2019_marked),0))
seq <- round(seq,-(nchar(seq)))

for (i in 2:nrow(deer_2019_marked)) {
  #Starting at the first row, assign an event ID that is (E+seq)
  deer_2019_marked$Event.ID[i-1]  <- paste0("E",format(seq, scientific = F)) 
  ...
  if(is.na(deer_2019_marked$duration[i]) | abs(deer_2019_marked$duration[i]) > mins){
    seq <- seq + 1 
  }
}
...
if(deer_2019_marked$duration[nrow(deer_2019_marked)] < mins| 
   is.na(deer_2019_marked$duration[nrow(deer_2019_marked)])){
  deer_2019_marked$Event.ID[nrow(deer_2019_marked)] <- deer_2019_marked$Event.ID[nrow(deer_2019_marked)-1] 
} else{
  deer_2019_marked$Event.ID[nrow(deer_2019_marked)] <- paste0("E",format(seq+1, scientific = F)) 
}
...
deer_2019_unmarked$Event.ID <- 9999
mins <- 30   # THIS IS THE DETECTION EVENT BREAK-POINT YOU CAN CHANGE
seq <- as.numeric(paste0(nrow(deer_2019_unmarked),0))
seq <- round(seq,-(nchar(seq)))

for (i in 2:nrow(deer_2019_unmarked)) {
  deer_2019_unmarked$Event.ID[i-1]  <- paste0("E",format(seq, scientific = F))
  if(is.na(deer_2019_unmarked$duration[i]) | abs(deer_2019_unmarked$duration[i]) > mins){
    seq <- seq + 1 
  }
}
...
# Update the information for the last row
# group ID  for the last row
if(deer_2019_unmarked$duration[nrow(deer_2019_unmarked)] < mins| 
   is.na(deer_2019_unmarked$duration[nrow(deer_2019_unmarked)])){
  deer_2019_unmarked$Event.ID[nrow(deer_2019_unmarked)] <- deer_2019_unmarked$Event.ID[nrow(deer_2019_unmarked)-1] 
} else{
  deer_2019_unmarked$Event.ID[nrow(deer_2019_unmarked)] <- paste0("E",format(seq+1, scientific = F)) 
}
...
write.csv(deer_2019_marked, file.path(output_directory, "marked_detections_fall_2019.csv"), row.names = FALSE)

```

Unmarked individuals are slightly different. We need to create a matrix of counts on each day for each site
```{r}
head(deer_2019_unmarked)
...
A<-merge(A, deer_2019_unmarked, by = c("trap.id", "occassion"), all.x = TRUE)
...
write.csv(deer_2019_unmarked, file.path(output_directory, "unmarked_matrix_fall_2019.csv"), row.names = FALSE)
```
