...
3. a camera operability matrix for October 2019
...
october_november_interval <-lubridate::interval(start = "2019-10-08", end = "2019-11-11")
...
camop_2019_figure<-ggplot(cam_deploy%>%
  mutate(new_start = case_when(start_date <= ymd("2019-10-08") ~ ymd("2019-10-08"),
         ...
         new_end = case_when(end_date > ymd("2019-11-11") ~ ymd("2019-11-11"),
                               end_date < ymd("2019-11-11") ~ end_date)), 
  aes(y = SiteID))+
  ...
  geom_vline(xintercept = ymd("2019-11-11"), linetype = "dashed", color = "red", linewidth = 1)+
  xlab("Date")+
  ylab("Site ID")+
  ggtitle("Fall 2019")
...
A<-cam_deploy%>%
  mutate(new_start = case_when(start_date <= ymd("2019-10-08") ~ ymd("2019-10-08"),
                               start_date > ymd("2019-10-08")  ~ start_date),
         new_end = case_when(end_date <= ymd("2019-11-11") ~ end_date,
                               end_date > ymd("2019-11-11")  ~ ymd("2019-11-11")),
...
cam_deploy<-cam_deploy %>%
  ...
  select(c(1,`2019-10-08`:`2019-11-11`))%>%
  replace(is.na(.), 0)
...
deer_2019<-deer_2019 %>%
  ...
  filter(date(datetime) >= "2019-10-08" & date(datetime) < "2019-11-11")
...
A<-cam_deploy%>%
  ...
  gather(key = "occassion", value = "active", 2:36)
