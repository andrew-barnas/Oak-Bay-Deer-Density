...
october_november_interval <-lubridate::interval(start = "2020-10-08", end = "2020-11-11")
...
camop_2020_figure<-ggplot(cam_deploy%>%
  mutate(new_start = case_when(start_date <= ymd("2020-10-08") ~ ymd("2020-10-08"),
         ...
         new_end = case_when(end_date > ymd("2020-11-11") ~ ymd("2020-11-11"),
                               end_date < ymd("2020-11-11") ~ end_date)), 
  aes(y = SiteID))+
  ...
  geom_vline(xintercept = ymd("2020-11-11"), linetype = "dashed", color = "red", linewidth = 1)+
  xlab("Date")+
  ylab("Site ID")+
  ggtitle("Fall 2020")
...
A<-cam_deploy%>%
  mutate(new_start = case_when(start_date <= ymd("2020-10-08") ~ ymd("2020-10-08"),
                               start_date > ymd("2020-10-08")  ~ start_date),
         new_end = case_when(end_date <= ymd("2020-11-11") ~ end_date,
                               end_date > ymd("2020-11-11")  ~ ymd("2020-11-11")),
...
cam_deploy<-cam_deploy %>%
  ...
  select(c(1,`2020-10-08`:`2020-11-11`))%>%
  replace(is.na(.), 0)
...
deer_2020<-deer_2020 %>%
  ...
  filter(date(datetime) >= "2020-10-08" & date(datetime) < "2020-11-11")
