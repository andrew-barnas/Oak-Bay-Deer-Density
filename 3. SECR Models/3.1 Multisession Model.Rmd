...
save(multisession_fit, file = file.path(output_directory, "multisession_fit.RData"))
...
I am going to use this chunk to do some comparisons amongst models
```{r}

#First, density ~ session vs density ~ 1 (null model?)
multisession_fit_density_stable<-secr.fit(deer_multisession,
                           model = D ~ 1, #lowercase "session" to fit a value for each level of session
                           mask = mask_2018,    #use the 2018 habitat mask with a large buffer.
                           details = list(autoini = 5), #Was having getting started unless I specified
                           trace = TRUE,
                           detectfn = "HN", #Half normal detection function
                            binomN = 0)  #counts modelled as poisson )
#Next allowing for a linear decrease in baseline detectability in each occassion?
# multisession_fit_detection_variable<-secr.fit(deer_multisession,
#                            model = list(D ~ session, g0 ~ T), #lowercase "session" to fit a value for each level of session
#                            mask = mask_2018,    #use the 2018 habitat mask with a large buffer.
#                            details = list(autoini = 5), #Was having getting started unless I specified
#                            trace = TRUE,
#                            detectfn = "HN", #Half normal detection function
...
#Next, based on those models we play with the detection function
#HR - hazard rate, #EX - exponential
# multisession_fit_hazard_rate<-secr.fit(deer_multisession,
#                            model = D ~ session, #lowercase "session" to fit a value for each level of session
#                            mask = mask_2018,    #use the 2018 habitat mask with a large buffer.
#                            details = list(autoini = 5), #Was having getting started unless I specified
#                            trace = TRUE,
#                            detectfn = "HR", #Half normal detection function
#                             binomN = 0)
# 
# multisession_fit_exponential<-secr.fit(deer_multisession,
#                            model = D ~ session, #lowercase "session" to fit a value for each level of session
#                            mask = mask_2018,    #use the 2018 habitat mask with a large buffer.
#                            details = list(autoini = 5), #Was having getting started unless I specified
#                            trace = TRUE,
#                            detectfn = "EX", #Half normal detection function
#                             binomN = 0)


save(multisession_fit_density_stable, file = file.path(output_directory, "multisession_fit_density_stable.RData"))
#save(multisession_fit_detection_variable, file = file.path(output_directory, "multisession_fit_detection_variable.RData"))
#save(multisession_fit_hazard_rate, file = file.path(output_directory, "multisession_fit_hazard_rate.RData"))
#save(multisession_fit_exponential, file = file.path(output_directory, "multisession_fit_exponential.RData"))


#Quick AIC comparison
summary(multisession_fit)
AIC(multisession_fit)

summary(multisession_fit_density_stable)
AIC(multisession_fit_density_stable)
#summary(multisession_fit_detection_variable)
#AIC(multisession_fit_detection_variable)

#summary(multisession_fit_exponential)
#AIC(multisession_fit_exponential)
#AIC(multisession_fit_hazard_rate)
...
#And make sure we save that data so we can start with it next time
save(deer_multisession, file = file.path(output_directory, "deer_multisession_females.RData"))
...
save(multisession_fit_suggested_buffer, file = file.path(output_directory, "multisession_fit_suggested_buffer_females.RData"))


```

```{r}
load(file = file.path(output_directory, "multisession_fit_suggested_buffer.RData"))

multisession_fit_suggested_buffer$details$chat

chat_res <- chat.nk(multisession_fit_suggested_buffer, nsim = 5000, ncores = 4, type = "Fletcher")
chat_res$chat       # numeric c-hat value
chat_res$stats      # diagnostic summary (X2/nu etc)
chat_res$p          # if nsim>0 gives p-value from simulations
```
...
This is janky, but rather than re-run everything I am going to upload the old and new density estimates so I can plot them together. I will fix it in post if need be
```{r}
both<-read.csv(file.path(output_directory, "density_estimates_old.csv"))%>%
  mutate(group = "male_and_female")

females<-read.csv(file.path(output_directory, "density_estimates.csv"))%>%
  mutate(group = "female_only")

df<-rbind(both, females)

ggplot(df, aes(x = as.factor(year), y = mean_density, color = group)) +
