...
globals[
  ...
  rep-id               ;needed for backend making this work in R
  absolute-path       ;path needed for R
]
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
to setup
  ...
  set output-filename (word absolute-path "deer_detections_" number-deer  "_rep" rep-id ".csv")

  ; Write the Header Row
  csv:to-file output-filename (list (list "camera_id" "xcor" "ycor" "tick" "deer_id" "sex" "number-deer" "rep-id"))
  ]
  ...
  ask cameras [
    ...
    if any? targets [
      ...
      ask targets [
        ...
        let new-row (list cam-id cam-x cam-y ticks who sex number-deer rep-id)
...
to camera-operability
 ; let operability-filename "camera_operability.csv"
  ...
  let operability-filename (word absolute-path "camera_operability_" number-deer "_rep" rep-id ".csv")

  ;Create/Overwrite the file with just the header row
  csv:to-file operability-filename (list (list "camera_id" "xcor" "ycor" "ticks" "rep-id" "number-deer"))
  ...
  ask cameras [
    ...
    let new-row (list cam-id cam-x cam-y ticks rep-id number-deer)
