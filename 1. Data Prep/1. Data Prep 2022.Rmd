...
deer_2022<-read.csv(file.path(input_directory, "detections_with_whid_2018-2022_updated_location/detections_with_whid_2018-2022_updated_location.csv" ))
...
#Before I filter out the September data for the deployment stuff, I need to extract the earliest and latest dates. Need it later for producing the list of sampling occassions and at this point I only want to deal with cameras deployed in September 2022
#So to do this I want to figure out if the start-end interval exists within the September 2022 interval
#First create an interval for September 2022
...
unique(deer_2022$true_location) %in% unique(camloc$SiteID)
unique(deer_2022$true_location) %in% unique(cam_deploy$SiteID)
...
#Similar, site 34a should also just be site34 according to Sandra
deer_2022<-deer_2022%>%
  mutate(true_location = if_else(str_detect(true_location, "Site34a"), "Site34", true_location))
...
#Sandra says that Site 12 should be Site 12b
deer_2022<-deer_2022%>%
  mutate(true_location = if_else(str_detect(true_location, "Site12"), "Site12b", true_location))


#THIS IS SOMETHING I WONT NEED ONCE SANDRA SENDS ME AN UPDATED SCRIPT BUT I WANT TO WORK THIS MORNING
#UPDATE THE DEPLOYMENT FILE FOR SITE 34
cam_deploy<-cam_deploy%>%
  mutate(SiteID = if_else(str_detect(SiteID, "Site34a"), "Site34", SiteID))



#I have 33 sites in the detection file. But how many in the location file?
n_distinct(deer_2022$SiteID)
...
unique(deer_2022$true_location) %in% camloc$SiteID
unique(deer_2022$true_location) %in% cam_deploy$SiteID
...
cam_deploy%>%
  mutate(new_start = case_when(start_date <= ymd("2022-09-01") ~ ymd("2022-09-01"),
         ...
         new_end = case_when(end_date >= ymd("2022-09-30") ~ ymd("2022-09-30"),
                               end_date <= ymd("2022-09-30") ~ end_date))
...
ggsave(filename = "camop_2022.jpeg", #name of file
...
cam_summary_2022<-cam_deploy%>%
 ...
 # group_by(SiteID)%>%
  summarise(n_cams = n_distinct(SiteID),
            ...
            min_n = min(n_days),
            max_n = max(n_days))
write.csv(cam_summary_2022, file.path(output_directory, "cam_summary_2022.csv"))
...
n_distinct(deer_2022$SiteID) #34 sites - so two had no detections?
n_distinct(cam_deploy$SiteID) #36 sites

#Remove rows where date is outside of September 2022
deer_2022<-deer_2022 %>%
  filter(datetime >= "2022-09-01" & datetime < "2022-10-01")
