...
deer_2021<-read.csv(file.path(input_directory, "detections_with_whid_2018-2022_updated_location/detections_with_whid_2018-2022_updated_location.csv" ))
...
#Before I filter out the September data for the deployment stuff, I need to extract the earliest and latest dates. Need it later for producing the list of sampling occassions and at this point I only want to deal with cameras deployed in September 2021
#So to do this I want to figure out if the start-end interval exists within the September 2021 interval
#First create an interval for September 2021
...
ggplot(cam_deploy, aes(y = SiteID))+
  ...
  xlab("Date")+
  ylab("Site ID")+
  ggtitle("Gannt chart of camera deployment in September 2021")


#Ok thats cool
#Lets take a look at how site names are formatted in each of the detection file, camera location, deployment files
unique(deer_2021$true_location)
unique(cam_deploy$SiteID)
unique(camloc$SiteID)

#ok lets check something. are the 2022 sites from the detection file in the deployment files? Looks like they are!
unique(deer_2021$true_location) %in% unique(camloc$SiteID)
unique(deer_2021$true_location) %in% unique(cam_deploy$SiteID)
...
#MISSING SITES IN THE DEPLOYMENT FILE
deer_2021%>%
  filter(!true_location %in% unique(cam_deploy$SiteID))%>%
  group_by(true_location)%>%
  summarise(n())
...
#Stopped here July 26th -Andrew
########################################################################


#Ok I think things look good, everything is about the same looking format?
#AUGUST 01/22 - I NEED TO SEE THE DATES FOR DETECTIONS IN SITE 07
ggplot(deer_2021%>%filter(true_location == "Site07"), aes(x = date(datetime)))+
  geom_bar(stat= "count")+
  geom_vline(xintercept = ymd("2021-09-01"), linetype = "dashed", color = "red")+
  geom_vline(xintercept = ymd("2021-09-16"), linetype = "dashed", color = "red")+
  xlab("Date")+
  ylab("Number of Observations")+
  ggtitle("Observations for Site07 \n(dashed red lines indcate camera deployment interval)")

A<-deer_2021%>%
  filter(true_location == "Site07")%>%
  mutate(Date = ymd(Date))%>%
  filter(Date > "2021-09-16")

A<-deer_2021%>%
  filter((true_location == "Site07" & Date > "2021-09-16"))
...
n_distinct(deer_2021$true_location)
...
unique(deer_2021$true_location) %in% camloc$SiteID
unique(deer_2021$true_location) %in% cam_deploy$SiteID
...
cam_deploy%>%
  mutate(new_start = case_when(start_date <= ymd("2021-09-01") ~ ymd("2021-09-01"),
         ...
         new_end = case_when(end_date >= ymd("2021-09-30") ~ ymd("2021-09-30"),
                               end_date <= ymd("2021-09-30") ~ end_date))
...
ggplot(cam_deploy, aes(y = SiteID))+
  ...
  xlab("Date")+
  ylab("Site ID")+
  xlim(c(ymd("2021-07-30"), ymd("2021-10-31")))+
  ggtitle("September 2021")

ggsave(filename = "camop_2021.jpeg", #name of file
...
cam_summary_2021<-cam_deploy%>%
 ...
 # group_by(SiteID)%>%
  summarise(n_cams = n_distinct(SiteID),
            ...
            min_n = min(n_days),
            max_n = max(n_days))
write.csv(cam_summary_2021, file.path(output_directory, "cam_summary_2021.csv"))
...
n_distinct(deer_2021$SiteID) #34 sites - so two had no detections?
n_distinct(cam_deploy$SiteID) #36 sites

#Remove rows where date is outside of September 2022
deer_2021<-deer_2021 %>%
  filter(datetime >= "2021-09-01" & datetime < "2021-10-01")
...
STOPPED HERE JULY 26TH FOR A BREAK TO WATCH SENSE8