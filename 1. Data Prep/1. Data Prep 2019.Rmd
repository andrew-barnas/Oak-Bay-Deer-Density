The purpose of this script is to prepare the 2019 deer detection data for density estimation using secr. The main ouputs of this script are:
1. a file of the marked detections in 2019
2. a file of the unmarked detections in 2019
3. a camera operability matrix for september 2019

The differences in this years data are that we are using October 2019 rather than Septmember as in the other years! This was due to logistic issues in tagging dear in September 2019

Andrew Barnas - April 2024
...
figure_directory<-reproducible::checkPath(file.path(getwd(), "1. Data Prep/figures"), create = TRUE)
...
deer_2019<-read.csv(file.path(input_directory, "September18-23_with_whid.csv"))



```

Quick look at the deer_2019 data
```{r}
deer_2019%>%
  #filter to 2019
  filter(df_year == 2019)%>%
...
#Another way to look at it is histograms?
deer_2019%>%
  #filter to 2019
  filter(df_year == 2019)%>%
  #read in datetimes
...
#I ALSO ONLY WANT STUFF FROM 2018 HERE
deer_2019<-deer_2019%>%
  select(-c(X, Unnamed..0, File, ImageQuality, Classifier, CommentsImages, Malfunction, DeleteFlag, RelativePath, Folder, Image.quality, Image.quality.1, Delete, Unnamed..31,))%>%
  filter(df_year == "2019")
...
str(deer_2019$datetime) #remember this is a datetime, not just a date. 
```


Continuing to slim down deployment frames and producing a camera operability plot
```{r}
...
#Before I filter out the OCTOBER 2019 data for the deployment stuff, I need to extract the earliest and latest dates. Need it later for producing the list of sampling occassions
#and at this point I only want to deal with cameras deployed in OCTOBER 2019
#So to do this I want to figure out if the start-end interval exists within the OCTOBER interval
#First create an interval for OCTOBER 2019
october_interval <-lubridate::interval(start = "2019-10-01", end = "2019-10-31")
#And then figure out if the start-end date interval overlaps October
#This is the list of cameras that we could possibly have a detection in October 2019
cam_deploy<-cam_deploy%>%
  mutate(deployment_interval = lubridate::interval(start =start_date, end = end_date))%>%
  filter(lubridate::int_overlaps(deployment_interval, october_interval))

#Gannt chart of camera deployment in October 2019

#Need to do something tricky to the dataframe. Basically need it so that if the camera started before Oct 1, we change the start date to Oct 1st, and if it ended after oct 31, we change it to oct 31


#I THINK I SAVE THIS AS A SUPPLEMENTAL MATERIAL??
camop_2019_figure<-ggplot(cam_deploy%>%
  mutate(new_start = case_when(start_date < ymd("2019-10-01") ~ ymd("2019-10-01"),
                               start_date > ymd("2019-10-01") ~ start_date),
         new_end = case_when(end_date > ymd("2019-10-31") ~ ymd("2019-10-31"),
                               end_date < ymd("2019-10-31") ~ end_date)), 
  aes(y = SiteID))+
  geom_segment(aes(x = new_start, xend = new_end,
                   y = SiteID, yend=SiteID))+
  geom_vline(xintercept = ymd("2019-10-01"), linetype = "dashed", color = "red", linewidth = 1)+
  geom_vline(xintercept = ymd("2019-10-31"), linetype = "dashed", color = "red", linewidth = 1)+
  xlab("Date")+
  ylab("Site ID")+
  ggtitle("October 2019")



#Output the camera operability matrix
ggsave(filename = "camera_operability_matrix_2019.jpeg", #name of file
       plot = camop_2019_figure,                 #plot you want to save
...
A<-cam_deploy%>%
  mutate(new_start = case_when(start_date <= ymd("2019-10-01") ~ ymd("2019-10-01"),
                               start_date > ymd("2019-10-01")  ~ start_date),
         new_end = case_when(end_date <= ymd("2019-10-31") ~ end_date,
                               end_date > ymd("2019-10-31")  ~ ymd("2019-10-31")),
...
cam_summary_2019<-cam_deploy%>%
  ...
  summarise(n_cams = n_distinct(SiteID),
            ...
            min_n = min(n_days),
            max_n = max(n_days))
write.csv(cam_summary_2019, file.path(output_directory, "camera_operability_summary_2019.csv"),
          row.names = FALSE)
...
unique(deer_2019$SiteID) %in% camloc$SiteID
unique(deer_2019$SiteID) %in% cam_deploy$SiteID
...
#Who is not in the deployment data?
deer_2019%>%
...
#Hmmmm lets take a look at these detections in more detail
missing_2019<-deer_2019%>%
  filter(!SiteID %in% unique(cam_deploy$SiteID))

#Yup so according to the Active Cameras data (see metadata folder), it should be camera 23a and 26b
#Lets replace those
deer_2019<-deer_2019%>%
  mutate(SiteID = str_replace(SiteID, pattern = "Site23", replacement = "Site23a"),
         SiteID = str_replace(SiteID, pattern = "Site26", replacement = "Site26b"))
#Checking here
unique(deer_2019$SiteID) %in% camloc$SiteID
unique(deer_2019$SiteID) %in% cam_deploy$SiteID
...
cam_deploy<-cam_deploy %>%
  ...
  select(c(1,`2019-10-01`:`2019-10-31`))%>%
  replace(is.na(.), 0)
...
write.csv(cam_deploy, file.path(output_directory, "operation_matrix_oct_2019.csv"), row.names = FALSE)
...
n_distinct(deer_2019$SiteID) #28 sites #I guess three sites had zero detections!
n_distinct(cam_deploy$SiteID) #31 sites

#Uh oh, what is the spread of data from 2019 detections?
deer_2019%>%
  summarise(min = min(datetime),
            max = max(datetime))


#Remove rows where date is outside of September 2018
deer_2019<-deer_2019 %>%
  filter(datetime >= "2019-10-01" & datetime < "2019-10-31")
...
unique(deer_2018_unmarked$WHID_all)
deer_2018_unmarked<-deer_2018_unmarked%>%
  mutate(WHID_all = "")
...
deer_2018_marked<-deer_2018_marked %>%
...
deer_2018_marked$Event.ID <- 9999
mins <- 30
...
#In this case, zero.
seq <- as.numeric(paste0(nrow(deer_2018_marked),0))
seq <- round(seq,-(nchar(seq)))

for (i in 2:nrow(deer_2018_marked)) {
  #Starting at the first row, assign an event ID that is (E+seq)
  deer_2018_marked$Event.ID[i-1]  <- paste0("E",format(seq, scientific = F)) 
  ...
  if(is.na(deer_2018_marked$duration[i]) | abs(deer_2018_marked$duration[i]) > mins){
    seq <- seq + 1 
  }
}
...
if(deer_2018_marked$duration[nrow(deer_2018_marked)] < mins| 
   is.na(deer_2018_marked$duration[nrow(deer_2018_marked)])){
  deer_2018_marked$Event.ID[nrow(deer_2018_marked)] <- deer_2018_marked$Event.ID[nrow(deer_2018_marked)-1] 
} else{
  deer_2018_marked$Event.ID[nrow(deer_2018_marked)] <- paste0("E",format(seq+1, scientific = F)) 
}
...
deer_2018_unmarked$Event.ID <- 9999
mins <- 30   # THIS IS THE DETECTION EVENT BREAK-POINT YOU CAN CHANGE
seq <- as.numeric(paste0(nrow(deer_2018_unmarked),0))
seq <- round(seq,-(nchar(seq)))

for (i in 2:nrow(deer_2018_unmarked)) {
  deer_2018_unmarked$Event.ID[i-1]  <- paste0("E",format(seq, scientific = F))
  if(is.na(deer_2018_unmarked$duration[i]) | abs(deer_2018_unmarked$duration[i]) > mins){
    seq <- seq + 1 
  }
}
...
# Update the information for the last row
# group ID  for the last row
if(deer_2018_unmarked$duration[nrow(deer_2018_unmarked)] < mins| 
   is.na(deer_2018_unmarked$duration[nrow(deer_2018_unmarked)])){
  deer_2018_unmarked$Event.ID[nrow(deer_2018_unmarked)] <- deer_2018_unmarked$Event.ID[nrow(deer_2018_unmarked)-1] 
} else{
  deer_2018_unmarked$Event.ID[nrow(deer_2018_unmarked)] <- paste0("E",format(seq+1, scientific = F)) 
}
...
write.csv(deer_2018_marked, file.path(output_directory, "marked_detections_sept_2018.csv"), row.names = FALSE)

```

Unmarked individuals are slightly different. We need to create a matrix of counts on each day for each site
```{r}
head(deer_2018_unmarked)
...
A<-merge(A, deer_2018_unmarked, by = c("trap.id", "occassion"), all.x = TRUE)
...
write.csv(deer_2018_unmarked, file.path(output_directory, "unmarked_matrix_sept_2018.csv"), row.names = FALSE)
```
