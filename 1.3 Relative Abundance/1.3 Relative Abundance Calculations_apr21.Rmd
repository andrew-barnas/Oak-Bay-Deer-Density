...
fawn_data<-fawn_data%>%
  rowwise()%>%
  filter((sum(Buck, 
              MarkedDoe, UnmarkedDoe, Doe_UnknownifMarked, Fawn, UnknownSex, Unknown_UnknownifMarked, UnknownAgeGroup, TotalAdult, TotalFawn, ICDoe_2019, ICDoe_2020, TotalMarked, na.rm = T) > 0))
...
fawn_data<-fawn_data%>%
  mutate(adults = Buck + 
           MarkedDoe + UnmarkedDoe + Doe_UnknownifMarked + UnknownSex + Unknown_UnknownifMarked + UnknownAgeGroup + TotalAdult + ICDoe_2019                       + ICDoe_2020 + TotalMarked,
...
#Information on the average number of independent detections for each group per camera
mean_detections<-A%>%
  group_by(year, SiteID, detection_type)%>%
  summarise(n_obs = n())%>%
  ungroup()%>%
  group_by(year, detection_type)%>%
  summarise(mean = mean(n_obs),
            sd = sd(n_obs))


#And a plot of those means with error
mean_detections_plot<-mean_detections%>%
  ggplot(aes(x = as.factor(year), y = mean, color = detection_type, group = detection_type))+
  geom_point(position = position_dodge(width = 0.3), 
             size = 3)+
  geom_errorbar(aes(ymin = mean  - sd, ymax = mean + sd), 
                position = position_dodge(width = 0.3), 
                width = 0.2)+
  scale_color_manual(labels = c("Adults Only", "Fawns Present"),
                      name = "Detection Type",
                      values = c("adult_only" = "#66A61E",
                                 "fawns_present" = "#E6AB02"))+
  ylab("Mean Independent Detections\nPer Camera\u00B1 SD ")+
  xlab("Year")+
  theme_classic()+
...
set.seed(420)
mean_detections_plot<-mean_detections_plot+
...
stacked_figure<-ggpubr::ggarrange(mean_detections_plot, relative_abundance_no_outliers,
                  nrow = 2,
                  labels = c("A", "B"),
                  font.label = list(size = 25))

#export those plots
ggsave(filename = "stacked_figure2.jpeg", #name of file
